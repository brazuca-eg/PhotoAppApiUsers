...
import com.kravchenko.photoapp.api.users.ui.model.LoginRequestModel;
...
//        SecretKey secretKey = new SecretKeySpec(secretKeyBytes, SignatureAlgorithm.HS512.getJcaName());
//
//        String token = Jwts.builder().subject(userDetails.getUserId())
//                .expiration(Date.from(Instant.now().plusMillis(Long.parseLong(
//                        Objects.requireNonNull(environment.getProperty("token.expiration_time"))))))
//                .issuedAt(Date.from(Instant.now())).signWith(secretKey, SignatureAlgorithm.HS512).compact();
//
//
//        res.addHeader("token", token);
//        res.addHeader("userId", userDetails.getUserId());
